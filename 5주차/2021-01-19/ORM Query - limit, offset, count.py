...
def _list():
    if(request.method == 'POST'):
        ...
        # 1번을 해보세요!
        member_list = Member.query.order_by(Member.age.desc()).offset(
            int(offset_num)).limit(int(limit_num))
        # 2번을 해보세요!
        cnt = Member.query.order_by(Member.age.desc()).offset(
            int(offset_num)).limit(int(limit_num)).count()
